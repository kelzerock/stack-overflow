export { isFullUserData } from './predicates/ifFullUserData';

export { isResponseHasData } from './predicates/isResponseHasData';
export { requestAuth } from './request/requestAuth';
export { isResponse } from './predicates/isResponse';
export { isResponseUserFull } from './predicates/isResponseUserFull';
export { requestLogin } from './request/requestLogin';
export { requestLogout } from './request/requestLogout';
export { isValidationError } from './predicates/isValidationError';
export { isAPIErrorResponse } from './predicates/isAPIErrorResponse';
export { requestRegistration } from './request/requestRegistration';
